...
{    
    ...
    $dep=$_POST['dep'];
    $dt=$_POST['dt'];
    $insgt=mysqli_query($dbcon,"INSERT INTO `g_timetable`(`cid`, `did`, `sem`, `dt`, `st`) VALUES ('$crs','$dep','$s','$dt','1')");
    $insid= mysqli_insert_id($dbcon);
    $selsubj=mysqli_query($dbcon,"select * from syllabus_data where crsid='$crs' and depid='$dep' and sem='$s'");
    if(mysqli_num_rows($selsubj)>0)
    {
        while($rsu=mysqli_fetch_row($selsubj))
        {
            $subid=$rsu[5];
            $d=date("D", strtotime($dt));
            if($d=="Sun")
            {
                $dt=date('Y-m-d H:i:s', strtotime($dt . ' +1 day'));
            }
            $insdt=mysqli_query($dbcon,"INSERT INTO `g_timedetails`(`gid`, `subid`, `dt`, `tim`, `st`) VALUES ('$insid','$subid','$dt','10:00','1')");
            $dt=date('Y-m-d H:i:s', strtotime($dt . ' +2 day'));
        }
    }
	...
	<body class="no-skin">
		...
		<div class="main-container" id="main-container">
			...
			<div id="sidebar" class="sidebar                  responsive">
				...
				<ul class="nav nav-list">
					...
					<li class="active">
						<a href="timetbl.php">
							<i class="menu-icon fa fa-file"></i>
							<span class="menu-text">Generate Time Table </span>
						</a>
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<!-- /.breadcrumb -->
                                                <div style="padding: 5px; margin-left: 15px;"><b>GENERATE TIME TABLE</b></div>
            ...
            function loaddep(x)
            {
                ...
                <tr>
                    <td colspan="2"><center><input type="submit" name="sub" value="Generate Time Table" class="btn btn-sm btn-danger" /></center></td>
                </tr>
                </table>
        </form>
                                                            <?php
                                                            $dt=date('Y-m-d');
                                                            $selx=mysqli_query($dbcon,"select * from g_timetable where dt>$dt");
                                                            ...
                                                            <table class="table table-bordered table-condensed table-responsive">
                                                                ...
                                                                <tr>
                                                                    <td colspan="5">
                                                                        ...
                                                                        echo"<center><b>Examination for $rcrs[1] - $rdep[2] - Semester $sem</b></center>";
                                                                        ?>
                                                                    </td>
                                                                </tr>
                                                                <?php
                                                                $seld=mysqli_query($dbcon,"select * from g_timedetails where gid='$rx[0]'");
                                                                $i=0;
                                                                ...
                                                                <tr>
                                                                    <td><?php echo $i ?></td>
                                                                    <td><?php echo $rd[2] ?></td>
                                                                    <td><?php echo date("d-M-Y",strtotime($rd[3])) ?></td>
                                                                    <td><?php echo $rd[4] ?></td>
                                                                </tr>
                                                                <?php
                                                                }
