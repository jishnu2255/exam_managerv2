...
if(isset($_POST['sub']))
{
    ...
    while($rr=  mysqli_fetch_row($selr))
    {
        ...
        $bn=0;//Bench Number
        $bn1=$nr1;
        $selstf1=mysqli_query($dbcon,"select * from staf_data where ealo <8 order by rand()");
        $rstf1=mysqli_fetch_row($selstf1);
        $stfid=$rstf1[2];
        $up=mysqli_query($dbcon,"update staf_data set ealo=ealo+1 where stfif='$stfid'");
        $insstalo=mysqli_query($dbcon,"INSERT INTO `staff_allocation`(`stid`, `eid`, `dt`, `tim`, `blk`, `room`) VALUES ('$stfid','".$_GET['eid']."','".$_GET['dt']."','".$_GET['ti']."','$blk','$rminm')");
        for($i=1;$i<=$nr;$i++)
        {
            $bn++;
            for($bb=1;$bb<=5;$bb++)
                                                   ...
                                                   <div class="col-xs-12">
                                                       ...
                                                       if(isset($_GET['dt']))
                                                       {
                                                            ...
                                                            while($rdata1=mysqli_fetch_row($seldata1))
                                                            {
                                                                ...
                                                                $selstud=mysqli_query($dbcon,"select * from exam_stud where eassign_id='$assid'");
                                                       ...
                                                       {
                                                           $eid=$_GET['eid'];
                                                           $chk=mysqli_query($dbcon,"select * from stf_alo_chk where eid='$eid'");
                                                           if(mysqli_num_rows($chk)>0)
                                                           {
                                                               ...
                                                               $up=mysqli_query($dbcon,"update staf_data set ealo='0'");
                                                               $insx=mysqli_query($dbcon,"INSERT INTO `stf_alo_chk`(`eid`) VALUES ('$eid')");
                                                           }
                                                           $i=0;
