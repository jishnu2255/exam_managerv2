...
if(isset($_POST['sub']))
{
    $dbt=  addslashes($_POST['dbt']);
    $insdbt=mysqli_query($dbcon,"INSERT INTO `stud_doubt`(`stud`, `stfid`, `dt`, `dbt`, `st`) VALUES ('$uid','".$_GET['sid']."','".date('Y-m-d')."','$dbt','0')");
    if($insdbt>0)
    {
        header("location:postdoubt.php?sid=".$_GET['sid']);
    }
}
?>
<!DOCTYPE html>
					...
					<ul class="nav ace-nav">
						...
						<li class="light-blue">
							<a data-toggle="dropdown" href="#" class="dropdown-toggle">
                                                            <img class="nav-user-photo" src="stud_pic/nopic.jpg" alt="Administrator" />
					...
					<div class="page-content">
						...
						<div class="row">
                                                    ...
                                                    <div class="col-xs-12">
                                                        ...
                                                        {
                                                            $sid=$_GET['sid'];
                                                            $selst=mysqli_query($dbcon,"select * from staf_data where stfif='$sid'");
                                                            $rst=mysqli_fetch_row($selst);
                                                        ?>
                                                        <div class="col-md-2"><center>
                                                            <img src="../admin/staff_pic/<?php echo $rst[10] ?>" class="img img-responsive" />
                                                            <b><?php echo $rst[1] ?></b>
                                                            </center>
                                                        </div>
                                                        <div class="col-sm-10">
                                                            <form method="post">
                                                                <b>POST YOUR DOUBT HERE</b>
                                                                <textarea rows="7" name="dbt" class="form-control"></textarea>
                                                            <input type="submit" name="sub" value="POST DOUBT" class="pull-right btn btn-sm btn-danger" />
                                                            </form>
                                                        </div>
                                                        <?php
                                                        }
                                                    ...
                                                    <div class="col-lg-12">
                                                        ...
                                                        if(isset($_GET['sid']))
                                                        {                                                            
                                                            $seldbt=mysqli_query($dbcon,"select * from stud_doubt where stud='$uid' and stfid='$sid' order by id desc");
                                                        }
                                                        else{
                                                            $seldbt=mysqli_query($dbcon,"select * from stud_doubt where stud='$uid' order by id desc");
                                                        }
                                                        if(mysqli_num_rows($seldbt)>0)
                                                        {
                                                            ?>
                                                        <table class="table table-bordered table-responsive">
                                                            ...
                                                            <?php
                                                            $i=0;
                                                            while($rdbt=mysqli_fetch_row($seldbt))
                                                            {
                                                                $i++;
                                                                ?>
                                                            <tr>
                                                                <td><?php echo $i ?></td>
                                                                <td><?php echo date("d-M-Y",  strtotime($rdbt[3])) ?></td>
                                                                <td><?php echo $rdbt[4] ?></td>
                                                                <td><span style="cursor: pointer;" class="label label-light" data-toggle="modal" data-target="#myModal<?php echo $i ?>"><span class="fa fa-exclamation-circle" style="color: green;"></span>
                                                                    <?php
                                                                    $ans1=mysqli_query($dbcon,"select * from dbt_ans where dbtid='$rdbt[0]'");
                                                                    echo mysqli_num_rows($ans1)." Answers Found"
                                                                    ?>
                                                                    </span>
                                                                ...
                                                                <div class="modal-dialog">
                                                                  ...
                                                                  <div class="modal-content">
                                                                    <div class="modal-header">
                                                                      ...
                                                                      <h4 class="modal-title"><?php echo $rdbt[4] ?></h4>
                                                                    </div>
                                                                    <div class="modal-body">
                                                                      <?php
                                                                      while($rans=mysqli_fetch_row($ans1))
                                                                      {
                                                                          echo $rans[2]."<hr />";
                                                                      }
                                                                      ?>
