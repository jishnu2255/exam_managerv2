...
if(isset($_POST['sub']))
{
    $crs=$_POST['crs'];
    $dep=$_POST['dep'];
    header("location:sdoubt.php?c=$crs&d=$dep");
}
?>
<!DOCTYPE html>
<html lang="en">
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="page-header">
							<h1>
								Dashboard :: Choose Your Staff And Post Doubts
						...
						<div class="row">
                                                    ...
                                                    <div class="col-lg-12">
                                                        <?php
                                                        $sel_subj=mysqli_query($dbcon,"select * from syllabus_data where crsid='$cid' and depid='$did' and sem='$sem'");
                                                        if(mysqli_num_rows($sel_subj)>0)
                                                        {
                                                            while($r_sub=mysqli_fetch_row($sel_subj))
                                                            {
                                                                $subj=$r_sub[5];
                                                                $chk=mysqli_query($dbcon,"select stf_id from subjct_assign where subnme='$subj'");
                                                                if(mysqli_num_rows($chk)>0)
                                                                {
                                                                    $rchk=mysqli_fetch_row($chk);
                                                                    $selst=mysqli_query($dbcon,"select * from staf_data where stfif='$rchk[0]' and desig='3'");
                                                                    $rst=mysqli_fetch_row($selst);
                                                                    ?>
                                                        <div class="col-lg-2">
                                                            <center>
                                                            <img src="../admin/staff_pic/<?php echo $rst[10] ?>" class="img img-responsive" style="height: 175px; border: 1px solid black; box-shadow: 0px 0px 5px black;" />
                                                            <b><?php echo $rst[1] ?></b><br />
                                                            <?php echo strtoupper($subj) ?><br />
                                                            <a href="postdoubt.php?sid=<?php echo $rst[2] ?>" target="_BLANK"><span class="label label-danger"><span class="fa fa-question-circle"></span> POST DOUBTS</span></a>
                                                            </center>
