...
if(isset($_POST['sub1']))
{
    ...
    $t=$_POST['t'];
    $descr=$_POST['de'];                
    $fn=date('d-m-Y-H-i-s').'-'.$_FILES['up']['name'];
    if(move_uploaded_file($_FILES['up']['tmp_name'], getcwd()."\\pquestions\\$fn"))
    {
        $ins_sm=mysqli_query($dbcon,"INSERT INTO `s_material`(`crs`, `dep`, `sem`, `subid`, `titl`, `descr`, `fil`, `up_by`, `st`) VALUES ('$c','$d','$sem','$subid','$t','$descr','$fn','$uid','2')");
        if($ins_sm>0)
        {
            header("location:pquestions.php?c=$c&d=$d&s=$sem&subid=$subid");
        }
    }
}
?>
	...
	<body class="no-skin">
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="page-header">
							<h1>
								Dashboard :: Upload Study Material
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <table class="table table-bordered table-condensed table-hover table-striped">
                <tr>
                    <td colspan="2"><center><b>ADD STUDY MATERIALS</b></center></td>                   
                </tr> 
                <tr>
                    <td>Subject Code</td>
                    <td><?php echo $_GET['subid'] ?></td>
                </tr>
                <tr>
                    <td>Title</td>
                    <td><input type="text" name="t" class="form-control" /></td>
                </tr>
                <tr>
                    <td>Description</td>
                ...
                <tr>
                    <td colspan="2">
                <center>
                    <input type="submit" name="sub1" value="ADD NOW" class="btn btn-sm btn-danger" />
                </center> 
                    </td>
                </tr>
            </table>            
            ...
            ?>
                <table class="table table-bordered table-condensed table-hover table-striped">
                    <tr>
                    <td colspan="2"><center><b>Choose Subject</b></center></td>                   
                </tr> 
                <tr>
                    <td>Course</td>
                    <td>
                ...
                {                    
                    ...
                    xmlhttp.open("GET", "loadsubject1.php?s=" + s+"&c="+c+"&d="+d, true);
                    xmlhttp.send();                                                            
                }
                </script>
                <tr>
            ...
            if(isset($_GET['subid']))
            {
            $selm=mysqli_query($dbcon,"select * from s_material where subid='".$_GET['subid']."' and st='2'");
            ...
            <table class="table table-bordered table-condensed table-hover table-responsive">
                ...
                <tr>
                    <td><?php echo $i ?></td>
                    <td style="width: 30%;"><?php echo $rm[5] ?></td>
                    <td><?php echo $rm[6] ?></td>
                    <td style="width: 20%;">
                        <?php
                        if($rm[8]==$uid)
                        {
                            ...
                            $selst=mysqli_query($dbcon,"select * from staf_data where stfif='$rm[8]'");
                            $rst=mysqli_fetch_row($selst);
                            echo $rst[1];
                        }
                        ?>
                    </td>
                    <td><a target="_BLANK" href="pquestions/<?php echo $rm[7] ?>" class="label label-warning"><span class="fa fa-download"></span></a></td>
                </tr>
                <?php
                }
                ?>
